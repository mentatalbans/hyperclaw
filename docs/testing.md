...
M5 acceptance commands, actual SDK/image/source evidence, the bounded Qwen demonstration and earlier failed attempts are recorded in [the M5 report](reports/2026-09-10-runtime-v2-m5.md) and [results JSON](reports/2026-09-10-runtime-v2-m5-results.json). Offline suites passed 332 tests on macOS Python 3.11/3.13 and local Linux, each with one expected base-only skip. Both Docker platforms passed 22 tests; all seven Qwen cases and 24 installed-wheel public tests passed. The genuine base wheel executed the missing-SDK check without a skip. Final whole-milestone review and cleanup are tracked in the report.
