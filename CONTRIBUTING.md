...
`make test-coverage` reports statement and branch counts separately; no percentage threshold substitutes for scenario coverage. CI uses locked dependencies, runs the quick battery once on Python 3.11 and 3.13, smoke-installs the wheel outside the checkout, and uploads reports even on failure. `make test-live` explicitly selects the installed model; `make test-all` combines quick and selected live checks with coverage. See docs/testing.md for retired features and remaining milestone gates.
