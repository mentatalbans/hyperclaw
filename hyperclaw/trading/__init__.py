"""
HyperClaw Trading Module — ATLAS_TRADING Integration
"""

from pathlib import Path
import httpx
import logging

logger = logging.getLogger("hyperclaw.trading")

ATLAS_TRADING_URL = "http://127.0.0.1:5001"
ATLAS_TRADING_ROOT = Path(str(Path.home() / ".hyperclaw/workspace/trading/trading"))


async def get_trading_status() -> dict:
    """Get the trading engine status."""
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            resp = await client.get(f"{ATLAS_TRADING_URL}/status")
            if resp.status_code == 200:
                return resp.json()
    except Exception as e:
        logger.warning(f"ATLAS_TRADING unreachable: {e}")
    return {"status": "offline", "funded": False, "standby": True}


async def send_signal(symbol: str, side: str, size_usd: float = 50, entry: float = 0) -> dict:
    """Send trading signal to ATLAS_TRADING."""
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                f"{ATLAS_TRADING_URL}/signal",
                json={
                    "symbol": symbol,
                    "side": side,
                    "size_usd": size_usd,
                    "entry": entry,
                }
            )
            return resp.json()
    except Exception as e:
        logger.error(f"Signal failed: {e}")
        return {"status": "error", "error": str(e)}


async def close_position(symbol: str = "", close_all: bool = False) -> dict:
    """Close position(s) on ATLAS_TRADING."""
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                f"{ATLAS_TRADING_URL}/close",
                json={"symbol": symbol, "close_all": close_all}
            )
            return resp.json()
    except Exception as e:
        logger.error(f"Close failed: {e}")
        return {"status": "error", "error": str(e)}


async def halt_trading() -> dict:
    """Emergency halt ATLAS_TRADING trading."""
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            resp = await client.post(f"{ATLAS_TRADING_URL}/halt")
            return resp.json()
    except Exception as e:
        return {"status": "error", "error": str(e)}


async def resume_trading() -> dict:
    """Resume ATLAS_TRADING trading."""
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            resp = await client.post(f"{ATLAS_TRADING_URL}/resume")
            return resp.json()
    except Exception as e:
        return {"status": "error", "error": str(e)}
